...
// import { Star, MapPin, Building2, ArrowRight, StarIcon } from "lucide-react";
...
// //    const reviewCount = properties._count?.reviews ?? properties.reviews?.length ?? 0;
// const reviewCount = properties.reduce((total: number, property: any) => {
//   const count = property._count?.reviews ?? property.reviews?.length ?? 0;
//   return total + count;
// }, 0);
            ...
            properties.map((property: any, index: number) => {
              ...
              return (
                <motion.div
                  ...
                  className="group flex flex-col bg-white rounded-2xl p-3 border border-slate-100 shadow-sm hover:shadow-xl hover:border-rose-100 transition-all duration-300"
                >
                  <Link href={`/properties/${activeId}`} className="relative aspect-[4/3] w-full rounded-xl overflow-hidden mb-4 bg-slate-50 block">
                    <img 
                      src={activeImage} 
                      alt={property.title} 
                      className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 ease-out" 
                  ...
                  <div className="px-1 pb-1 flex-1 flex flex-col justify-between space-y-3">
                    <div className="space-y-1">
                      ...
                      <p className="flex items-center gap-1 text-slate-400 text-xs font-medium">
                        <MapPin className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                        <span className="truncate">{property.location || "Sylhet, Bangladesh"}</span>
                      </p>
                    </div>
                    ...
                    <div className="pt-2.5 border-t border-slate-50 flex items-center justify-between">
                      ...
                      <Link href={`/properties/${activeId}`} className="p-2 rounded-full bg-slate-50 text-slate-600 group-hover:bg-[#FF385C] group-hover:text-white transition-all duration-300">
                        <ArrowRight className="w-3.5 h-3.5 stroke-[2.5px]" />
                      </Link>
                    </div>
                  </div>
