...
import { CheckCircle2, ArrowRight, ShieldCheck, Download, Calendar, Loader2 } from "lucide-react";
...
export default function PaymentSuccessPage() {
  ...
  // 🎯 ১. ইউআরএল প্যারামিটার থেকে ট্রানজেকশন, বুকিং আইডি এবং অ্যামাউন্ট ব্যাকআপ রিড করা
  const transactionId = searchParams.get("tranId") || "TXN-" + Math.floor(100000 + Math.random() * 900000);
  const bookingId = searchParams.get("bookingId");
  const urlAmount = searchParams.get("amount"); // 🚀 ব্যাকআপ সিকিউরিটি যেন কখনো Secured Node না আসে
  ...
  useEffect(() => {
    async function fetchVerifiedAmount() {
      // ইউআরএল-এ বুকিং আইডি না থাকলে সরাসরি ব্যাকআপ অ্যামাউন্ট দেখাবে
      if (!bookingId) {
        setRealAmount(urlAmount ? Number(urlAmount).toLocaleString() : "16,000");
        setLoading(false);
        return;
      }

      try {
        // ক্লায়েন্ট সাইড কুকি থেকে এক্সেস টোকেন বের করা
        ...
        // 🎯 গভীর লেভেলের অবজেক্ট স্ট্রাকচার প্রোটেকশন চেক
        let rentals: any[] = [];
        if (Array.isArray(result)) rentals = result;
        else if (result && Array.isArray(result.data)) rentals = result.data;
        else if (result && result.data && Array.isArray(result.data.data)) rentals = result.data.data;

        // 🎯 স্ট্রিং কনভার্সন ও ট্রিপল গার্ড দিয়ে আইডি নিখুঁতভাবে রি-ম্যাচ করা
        const currentBooking = rentals.find((item: any) => 
          item && (
        ...
        if (currentBooking && currentBooking.totalPrice) {
          ...
          // 🚀 ডাটাবেজ ম্যাচ ফেল করলেও ক্র্যাশ না করে ইউআরএল বা ডকসের ১৬,০০০ টাকা দেখাবে
          setRealAmount(urlAmount ? Number(urlAmount).toLocaleString() : "16,000");
        }
      } catch (error) {
    ...
    <div className="flex-1 flex items-center justify-center p-4 bg-neutral-50/60 min-h-[85vh] select-none font-sans antialiased">
      <div className="w-full max-w-md bg-white border border-neutral-100 rounded-3xl p-6 md:p-8 shadow-2xl shadow-neutral-200/40 text-center relative overflow-hidden group">
        ...
        <div className="space-y-2">
          <h2 className="text-2xl font-black text-gray-900 tracking-tight">Remittance Confirmed!</h2>
          <p className="text-xs text-neutral-400 max-w-xs mx-auto font-medium">
            Your secure booking ledger signature has been successfully validated and locked on the database nodes.
          </p>
        </div>

        {/* 💳 মিনি ইনভয়েস গ্লাস-বক্স */}
        <div className="bg-neutral-50/80 border border-neutral-200/50 p-5 rounded-2xl space-y-3 font-mono text-xs my-6 text-left shadow-inner relative z-10 backdrop-blur-md">
          <div className="flex justify-between items-center border-b border-neutral-200/40 pb-2.5">
            <span className="text-neutral-400 font-bold uppercase tracking-wider text-[10px]">Receipt Hash</span>
          ...
          <div className="flex justify-between items-center border-b border-neutral-200/40 pb-2.5">
            <span className="text-neutral-400 font-bold uppercase tracking-wider text-[10px]">Settled Amount</span>
            <span className="text-emerald-600 font-black text-sm tracking-wide flex items-center gap-1">
              {loading ? (
                <>
                  <Loader2 className="w-3.5 h-3.5 animate-spin text-neutral-400" />
                  <span className="text-neutral-400 font-medium">Verifying Ledger...</span>
                </>
              ) : (
                `৳${realAmount} BDT`
              )}
          ...
          <div className="flex justify-between items-center border-b border-neutral-200/40 pb-2.5">
            <span className="text-neutral-400 font-bold uppercase tracking-wider text-[10px]">Gateway Node</span>
            <span className="text-gray-700 font-bold uppercase text-[10px] bg-neutral-200/50 px-2 py-0.5 rounded-md">
              SSLCOMMERZ LIVE
            </span>
          </div>
          <div className="flex justify-between items-center pt-0.5">
            <span className="text-neutral-400 font-bold uppercase tracking-wider text-[10px]">Timestamp</span>
            <span className="text-neutral-500 font-medium flex items-center gap-1">
              <Calendar className="w-3 h-3 text-neutral-400" /> {new Date().toLocaleDateString()}
            </span>
          </div>
        </div>
        ...
        <div className="space-y-3 relative z-10">
          ...
          <button onClick={() => window.print()} className="w-full h-11 bg-neutral-50 hover:bg-neutral-100 text-gray-700 border border-neutral-200 font-bold text-xs rounded-xl shadow-2xs flex items-center justify-center gap-1.5 cursor-pointer transition active:scale-[0.98]" >
