...
export default function DashboardSidebar({ user }: { user: any }) {
  ...
  const currentMenuItems =
    menuConfig[role as keyof typeof menuConfig] || menuConfig.TENANT;
  ...
  return (
    // 🎯 ফিক্স ১: 'top-[73px]' দিয়ে ন্যাভবারের ফাঁকা অংশ বন্ধ করা হয়েছে এবং 'h-[calc(100vh-73px)]' দিয়ে নিচের কালো দাগ ও স্ক্রোল হাইট নিখুঁত করা হয়েছে। mt-4 ফেলে দেওয়া হয়েছে।
    <aside className="w-72 bg-white border-r border-neutral-100 p-6 hidden md:flex flex-col justify-between shrink-0 sticky top-[73px] h-[calc(100vh-73px)] select-none dark:bg-slate-900 dark:border-slate-800/60 transition-colors duration-300">
      <div className="space-y-8 overflow-y-auto pr-1 -mr-1 scrollbar-none">
        
        {/* প্রোফাইল কার্ড */}
        <div className="px-5 py-4 bg-rose-50/50 border border-rose-100/50 rounded-[24px] relative overflow-hidden group dark:bg-rose-950/20 dark:border-rose-900/30">
          <div className="absolute -right-2 -top-2 w-12 h-12 bg-rose-100 rounded-full blur-2xl opacity-50 group-hover:opacity-100 transition-opacity dark:bg-rose-900" />
          <p className="text-[10px] font-black uppercase tracking-[0.15em] text-rose-500/80 dark:text-rose-400">
            Authorized Access
          </p>
          <h4 className="text-base font-black text-gray-900 mt-1 tracking-tight dark:text-slate-100">
            {panelTitle}
          </h4>
          <p className="text-[11px] font-medium text-gray-500 truncate mt-1 dark:text-slate-400">
              ...
              <Link
                ...
                className={`flex items-center justify-between px-4 py-3.5 rounded-2xl text-[13px] font-bold transition-all duration-300 group ${
                  isActive
                    ...
                    : "text-gray-500 hover:bg-neutral-50 hover:text-rose-500 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-rose-400"
                }`}
              >
                <div className="flex items-center gap-3">
                  <Icon
                    className={`w-4.5 h-4.5 ${
                      isActive
                        ? "text-white"
                        : "text-gray-400 group-hover:text-rose-500 dark:text-slate-500 dark:group-hover:text-rose-400"
                    }`}
                  />
                  <span>{item.label}</span>
                </div>
      ...
      {/* 🎯 ফিক্স ২: নিচের বর্ডারের কালার স্লট-৮০০ করা হয়েছে যাতে ডার্ক মোডে কোনো বিশ্রী কালো দাগ না আসে */}
      <div className="space-y-2 border-t border-neutral-100 pt-4 dark:border-slate-800 mt-auto">
        <Link
          href="/"
          className="flex items-center gap-3 px-4 py-3 rounded-2xl text-[13px] font-bold text-gray-500 hover:bg-neutral-50 hover:text-gray-900 transition-all group dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-slate-200"
        >
          <Home className="w-4.5 h-4.5 text-gray-400 group-hover:text-gray-900 dark:text-slate-500 dark:hover:text-slate-300" />
          <span>Back to Home</span>
        </Link>
        
        <button
          onClick={handleLogout}
          className="w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-[13px] font-black text-rose-600 hover:bg-rose-50 transition-all cursor-pointer dark:text-rose-400 dark:hover:bg-rose-950/20"
        >
          <LogOut className="w-4.5 h-4.5" />
          <span>Sign Out</span>
        </button>
