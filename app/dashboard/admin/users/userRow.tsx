...
export default function UserRow({ user }: { user: any }) {
  ...
  const handleStatusUpdate = async (newStatus: "ACTIVE" | "BLOCKED") => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(
          `User successfully ${newStatus === "ACTIVE" ? "Activated" : "Blocked"}!`,
        );
    ...
    >
      ...
      >
        <div className="col-span-1 lg:col-span-4 flex items-center gap-3">
          <div
            className={`p-2.5 rounded-xl ${currentStatus === "BLOCKED" ? "bg-red-50 text-red-500" : "bg-rose-50 text-rose-500"}`}
          >
        ...
        <div className="col-span-1 lg:col-span-2 text-left lg:text-center">
          <span
            className={`px-3 py-1 rounded-full text-[11px] font-black ${currentStatus === "ACTIVE" ? "bg-emerald-50 text-emerald-600 border border-emerald-100" : "bg-red-50 text-red-600 border border-red-100"}`}
          >
            {currentStatus}
          </span>
        </div>
        ...
        <div className="col-span-1 lg:col-span-1 text-right flex items-center justify-end gap-2">
          {isExpanded ? (
            ...
            <Eye className="w-4 h-4 text-slate-400 group-hover:text-rose-400" />
          )}
        </div>
      </div>
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mt-3 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
            <div className="flex flex-wrap gap-4 text-xs font-medium text-slate-500">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>
                  Privilege Level:{" "}
                  <strong className="text-slate-700">{user.role}</strong>
                </span>
              </div>
            </div>
