...
export default function CategoryRow({ category }: { category: any }) {
  ...
  const propertyCount =
    category.properties?.length || category._count?.properties || 0;
        ...
        <div className="col-span-1 lg:col-span-5 flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-slate-800 text-sm">
              {category.name || "General Tag"}
            </h3>
          </div>
        </div>

        <div className="col-span-1 lg:col-span-4 text-left lg:text-center">
          <span className="lg:hidden font-bold text-slate-400 text-[11px] uppercase tracking-wider mr-2">
            Listings:
          </span>
          <span
            className={`px-2.5 py-0.5 rounded-lg text-xs font-black ${propertyCount > 0 ? "bg-blue-50 text-blue-600 border border-blue-100" : "bg-slate-100 text-slate-400"}`}
          >
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mt-3 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  router.push(`/properties?search=${category.name}`);
                }}
              ...
              <button
                ...
                onClick={(e) => {
                  ...
                  toast.error("Remove Category?", {
                    description:
                      "This will completely delete this category registry from the cloud database.",
                    duration: Infinity,
                    action: {
                      label: "Confirm",
                      onClick: () => {
