...
export default function WelcomeBanner({ userRole, userName }: BannerProps) {
  const getRoleTheme = () => {
    if (userRole === 'ADMIN') return {
      ...
      tagline: 'Global overview of RentNest infrastructure, platform analytics, and security audits.',
      icon: <ShieldCheck className="w-5 h-5 text-indigo-400" />
    };
    if (userRole === 'LANDLORD') return {
      title: 'Premium Landlord',
      gradient: 'from-amber-600 via-orange-600 to-rose-700',
      tagline: 'Track your property performances, lease requests, and monthly rental revenue updates.',
      icon: <Sparkles className="w-5 h-5 text-amber-300" />
    };
    return {
      title: 'Verified Tenant',
      gradient: 'from-rose-500 via-pink-500 to-purple-600',
      tagline: 'Explore active listings, monitor your rental bookings, and review billing invoices.',
  ...
  return (
    <div className={`relative overflow-hidden rounded-3xl bg-gradient-to-r ${theme.gradient} text-white p-8 md:p-10 shadow-xl shadow-slate-900/10 transition-all duration-700 transform hover:scale-[1.002]`}>
