...
import { getMyProperties, deleteProperty, toggleAvailability } from "../../_actions/landlordAction";
import { toast } from "sonner";
import { Trash2, RefreshCw, MapPin, Eye, Edit3, Plus, Tag, ArrowUpRight } from "lucide-react";
    ...
    <div className="space-y-6 p-4 md:p-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-neutral-100 pb-5">
        <div>
          ...
          <p className="text-xs text-neutral-500">Live grid-less operational registry of your assets.</p>
        </div>
        <Link 
      ...
      {properties.length === 0 ? (
        <div className="text-center py-16 border border-dashed rounded-2xl text-neutral-400 text-sm bg-neutral-50/30">
          Your inventory is completely empty. Launch a new listing to populate this database registry!
        </div>
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-sm">
              ...
              <tbody className="divide-y divide-neutral-50 text-neutral-700 font-medium">
                {properties.map((item) => (
                  <tr key={item._id || item.id} className="hover:bg-neutral-50/40 transition-colors">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className="inline-flex items-center text-[10px] text-neutral-400 uppercase tracking-wide gap-0.5 mt-0.5">
                            ...
                            {typeof item.categoryId === "object" ? item.categoryId.name : "Asset Listed"}
                          </span>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-1 text-xs text-neutral-600">
                        ...
                        <span className="line-clamp-1">{item.location || "N/A"}, {item.city || "N/A"}</span>
                      </div>
                    </td>
                    ...
                    <td className="p-4">
                      <p className="font-black text-gray-900 text-sm">
                        ৳{item.pricePerDay}<span className="text-[10px] font-normal text-neutral-400">/day</span>
                      </p>
                    </td>
...
<td className="p-4">
  ...
  onClick={() => handleToggle(item._id || item.id, item.isAvailable)} 
  className={`px-2.5 py-1 text-[11px] font-black rounded-lg border uppercase tracking-wider flex items-center gap-1 transition-all ${ 
    item.isAvailable !== false 
      ? "bg-emerald-50 text-emerald-700 border-emerald-100 hover:bg-emerald-100" 
  ...
  {item.isAvailable !== false ? "Available" : "Not Available"}
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {/* Update / Edit Button */}
                        <Link 
                          href={`/dashboard/landlord/properties/edit/${item._id || item.id}`}
                        ...
                        <button 
                          onClick={() => promptDelete(item._id || item.id, item.title)}
